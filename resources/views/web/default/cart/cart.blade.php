                                ...
                                <div class="col-8">
                                    <div class="webinar-card-body p-0 w-100 h-100 d-flex flex-column">
                                        <div class="d-flex align-items-center justify-content-between">
                                            ...
                                            @elseif(!empty($cart->reserve_meeting_id))
                                                <h3 class="font-16 font-weight-bold text-dark-blue">{{ trans('meeting.reservation_appointment') }} {{ (!empty($cart->reserveMeeting->student_count) and $cart->reserveMeeting->student_count > 1) ? '('. trans('update.reservation_appointment_student_count',['count' => $cart->reserveMeeting->student_count]) .')' : '' }}</h3>
                                            @endif
                                        </div>
                        ...
                        <div class="col-6 col-lg-2 d-flex flex-md-column align-items-center justify-content-center">
                            ...
                            @if(!empty($cart->webinar_id))
                                @if($cart->webinar->getDiscount($cart->ticket))
                                    <span class="text-gray text-decoration-line-through mx-10 mx-md-0">{{ handlePrice($cart->webinar->price) }}</span>
                                    <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cart->webinar->price - $cart->webinar->getDiscount($cart->ticket)) }}</span>
                                @else
                                    <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cart->webinar->price) }}</span>
                                @endif

                            @elseif(!empty($cart->reserve_meeting_id))
                                <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cart->reserveMeeting->paid_amount) }}</span>
                            @endif
                        </div>
